...
class YamahaRX(eg.PluginClass):
    def __init__(self):
        ...
        self.AddAction(SetInitVolume, clsName="Set Initial Volume", description="Set the initial volume when the receiver turns on (zone specific).  The mode controls if the initial volume will retain last volume or reset to the given volume.")
        self.AddAction(SetMaxVolume, clsName="Set Max Volume", description="Set the max volume the zone can go.")
        ...
        self.AddAction(RadioSetExact, clsName="Radio Set Station", description="Set an exact radio station to go to.")
        self.AddAction(InputVolumeTrim, clsName="Source Volume Trim", description="Each source volume trim (-6.0 dB to 6.0 dB).")
        ...
        self.AddAction(SendAnyCommand, clsName="Send Any Command", description="Sends any command in YNC.")
